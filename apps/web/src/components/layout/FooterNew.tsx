...
export function FooterNew({ locale }: FooterProps) {
  ...
  const footerLinks = {
    ...
    students: {
      title: locale === 'ru' ? 'Студентам' : 'Talabalar uchun',
      links: [
    ...
    support: {
      ...
      links: [
        { name: locale === 'ru' ? 'Помощь' : 'Yordam', href: `/${locale}/help` },
        { name: locale === 'ru' ? 'FAQ' : 'Savol-javob', href: `/${locale}/faq` },
        { name: locale === 'ru' ? 'Связаться с нами' : 'Biz bilan bog\'laning', href: `/${locale}/contact` },
        { name: locale === 'ru' ? 'Статус системы' : 'Tizim holati', href: `/${locale}/health` },
      ],
    },
    legal: {
      title: locale === 'ru' ? 'Юридическая информация' : 'Huquqiy ma\'lumot',
      links: [
        { name: locale === 'ru' ? 'Конфиденциальность' : 'Maxfiylik', href: `/${locale}/privacy` },
        { name: locale === 'ru' ? 'Условия использования' : 'Foydalanish shartlari', href: `/${locale}/terms` },
        { name: locale === 'ru' ? 'Cookies' : 'Cookies', href: `/${locale}/cookies` },
      ],
    },
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="mt-4 text-sm text-gray-600 max-w-md">
              {locale === 'ru' 
                ? 'Лучшие скидки и предложения для студентов Узбекистана. Экономьте на покупках, развлечениях и образовании.'
                : 'O\'zbekiston talabalari uchun eng yaxshi chegirmalar va takliflar. Xaridlar, o\'yin-kulgi va ta\'limda tejang.'
              }
            </p>
        ...
        <div className="mt-12 pt-8 border-t border-gray-200">
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="mt-4 md:mt-0 flex space-x-6">
              <Link href={`/${locale}/privacy`} className="text-sm text-gray-500 hover:text-brand-600">
                {locale === 'ru' ? 'Конфиденциальность' : 'Maxfiylik'}
              </Link>
              <Link href={`/${locale}/terms`} className="text-sm text-gray-500 hover:text-brand-600">
